...
func init() {
	// Audax mainnet Address encoding magics
	MainNetParams = chaincfg.MainNetParams
	MainNetParams.Net = MainnetMagic
	MainNetParams.PubKeyHashAddrID = []byte{23} // starting with 'A'
	MainNetParams.ScriptHashAddrID = []byte{8}
	MainNetParams.PrivateKeyID = []byte{142}

	// Audax testnet Address encoding magics
	TestNetParams = chaincfg.TestNet3Params
	TestNetParams.Net = TestnetMagic
	TestNetParams.PubKeyHashAddrID = []byte{58} // starting with 'Q'
	TestNetParams.ScriptHashAddrID = []byte{66}
	TestNetParams.PrivateKeyID = []byte{56}
}
...
func (p *AudaxParser) ParseBlock(b []byte) (*bchain.Block, error) {
	...
	if h.Version > 3 {
		// Skip past AccumulatorCheckpoint which was added in Audax block version 4
...
func (p *AudaxParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
